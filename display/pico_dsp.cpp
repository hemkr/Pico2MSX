...
// Include appropriate display backend
#ifdef USE_LIBDVI
// Waveshare: PicoDVI backend
#include "display_backend.h"
#else
// FruitJam/Olimex: HSTX backend
...
gfx_error_t PICO_DSP::begin(gfx_mode_t mode)
{
  ...
  switch(mode) {
    case MODE_VGA_320x240:
      ...
      fb_stride = 320;
      #ifdef USE_LIBDVI
        display_backend_init(fb_width, fb_height);
        fb0 = (uint8_t *)display_backend_get_framebuffer();
      #else
...
#define DEBUG_I2C (0)
static bool i2c_available = true;  // Track if I2C codec is available

static void writeRegister(uint8_t reg, uint8_t value) {
  if (!i2c_available) return;  // Skip if I2C not available
  ...
  if (res != 2) {
    printf("WARNING: i2c_write_timeout failed (codec not present?): res=%d\n", res);
    i2c_available = false;  // Disable further I2C attempts
    return;
  }
if(DEBUG_I2C)
...
static uint8_t readRegister(uint8_t reg) {
  if (!i2c_available) return 0xFF;  // Return dummy value if I2C not available
...
if(DEBUG_I2C)
printf("res=%d\n", res);
    printf("WARNING: i2c_read_timeout failed (codec not present?): res=%d\n", res);
    i2c_available = false;
    return 0xFF;
  }
  uint8_t value = buf[0];
...
{
  printf("Initializing I2S audio (codec may not be present on all boards)...\n");
...
if(DEBUG_I2C)
  ...
  // Reset codec (will fail gracefully if codec not present)
  writeRegister(0x01, 0x01);
  
  if (!i2c_available) {
    printf("I2S codec not detected - audio will use PWM fallback\n");
    return;  // Skip codec configuration if not available
  }
